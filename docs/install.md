...
3. [Click here](https://github.com/Anatdx/IcePatch/releases) to get the latest stable version of IcePatch Manager.
...
2. IcePatch always patch the `boot.img` of any device. Don't attempt to patch or flash the `init_boot` or other partition image files. The IcePatch developers aren't responsible for the failure of resulting patch and boot.
...
IcePatch installation requirements are mainly reflected in the kernel configuration. Here are the requirements of kernel:
...
There are several ways to patch IcePatch.

### Automatically patching {#automatically-patching}

1. Download the latest version of IcePatch Manager from [GitHub](https://github.com/Anatdx/IcePatch/releases).
...
::: warning
It's strictly prohibited to set weak keys like `12345678`. The latest versions of IcePatch [require the use of strong keys](/warn).
:::
...
When the KernelPatch is updated and IcePatch Manager remains unchanged, you can choose to manually patch the kernel.
...
::: info
You can click [here](https://exame.icepatch.anatdx.com/) to try it out.
:::
...
The latest version of IcePatch Manager supports directly flashing via third-party Recovery, such as TWRP.

::: warning
Directly flashing was first introduced in version `10888`, and earlier versions of IcePatch Manager **DO NOT** support this method.
:::

Change the IcePatch Manager file suffix from `.apk` to `.zip`. For example:
...
After doing this, you can flash this `.zip` file via third-party Recovery's Flash function. IcePatch will be automatically installed, just like Magisk.
...
If you need to customize the SuperKey, go to IcePatch Manager after booting and repatch to reset it.
:::
...
::: warning
Automatically uninstall was first introduced in version `10888`, and earlier versions of IcePatch Manager **DO NOT** support this method.
:::

Change the IcePatch Manager file suffix from `.apk` to `.zip` and add modify file name to anything with `uninstall`. For example:
...
After doing this, you can flash this `.zip` file via third-party Recovery's Flash function. IcePatch will be automatically removed, just like Magisk.
