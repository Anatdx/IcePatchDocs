...
IcePatch 依赖于 KernelPatch，继承了其全部功能并进行了扩展。因此，IcePatch 也支持使用 KernelPatch Modules (KPM)。下面是关于 KPM 的使用说明。
...
::: tip 关于“安装”
目前，IcePatch 并没有实现 KPM 的“安装”操作，因此现阶段 IcePatch 的 KPM 只能通过“嵌入”和“加载”两种方式安装。KernelPatch 和 IcePatch 的开发者 正在努力实现相关功能，请耐心等待。
:::
...
KPM 的嵌入可以在首次使用 IcePatch 管理器修补 `boot.img` 时或者在完成 IcePatch 安装后进行。你也可以使用 `kptools` 手动嵌入。
...
在 IcePatch 安装后，嵌入 KPM 的方式和安装 APM 的方式大致相同。你可以在“内核模块”界面点击右下角的按钮，选择“嵌入”，之后的步骤可以参考[首次修补时嵌入](/zh_CN/kpm-usage-guide#embed-kpms-at-first-patching)进行。
...
::: tip 注意
目前 KernelPatch 和 IcePatch 还未实现 KPM 的安装，以下的描述仅为对安装 KPM 的预期行为描述。
:::
