...
IcePatch fornece um mecanismo de módulo (AndroidPatch Module) que consegue modificar o diretório do sistema enquanto mantém a integridade da partição do sistema. Este mecanismo é conhecido como `sem sistema`.

A implementação de módulos do IcePatch foi copiada e modificada do [KernelSU](https://github.com/tiann/KernelSU).
...
3. Suporte ao SELinux, IcePatch usa diretamente o `magiskpolicy`

O mecanismo de módulos do IcePatch é quase o mesmo do Magisk. Se você está familiarizado com o desenvolvimento de módulos Magisk, o desenvolvimento de módulos IcePatch é muito semelhante. Você pode pular a introdução dos módulos abaixo e só precisa ler quais são as diferenças.

[[toc]]

## BusyBox

O IcePatch vem com um recurso binário BusyBox completo (incluindo suporte completo ao SELinux). O executável está localizado em `/data/adb/ap/bin/busybox`.
O BusyBox do IcePatch suporta "ASH Standalone Shell Mode" alternável em tempo de execução.
...
Cada script shell executado no contexto do IcePatch será executado no shell `ash` do BusyBox com o Modo Autônomo ativado. Para o que é relevante para desenvolvedores terceirizados, isso inclui todos os scripts de inicialização e scripts de instalação de módulos.

Para aqueles que desejam usar o recurso Modo Autônomo fora do IcePatch, existem 2 maneiras de ativá-los:
...
Para garantir que todos os shells `sh` subsequentes executados também sejam executados no Modo Autônomo, a opção 1 é o método preferido (e é isso que o IcePatch e o gerenciador do IcePatch usam internamente), pois as variáveis ​​de ambiente são herdadas para os subprocesso.
...
O BusyBox do IcePatch agora está usando o arquivo binário compilado diretamente do projeto Magisk. **Obrigado ao Magisk!** Portanto, você não precisa se preocupar com problemas de compatibilidade entre scripts BusyBox no Magisk e IcePatch porque eles são exatamente iguais!
:::

## APMódulo {#IcePatch-modules}

Um módulo IcePatch é uma pasta colocada em `/data/adb/modules` com a estrutura abaixo:
...
|   ├── uninstall.sh        <--- Este script será executado quando o IcePatch remover seu módulo
|   ├── action.sh           <--- Este script será executado quando o usuário clicar no botão Ação no IcePatch
...
::: tip DIFERENÇAS COM MAGISK
O IcePatch não possui suporte integrado para o Zygisk, portanto não há conteúdo relacionado ao Zygisk no módulo.
...
`module.prop` é um arquivo de configuração para um módulo. No IcePatch, se um módulo não contiver este arquivo, ele não será reconhecido como um módulo. O formato deste arquivo é o seguinte:
...
Você pode determinar se o script está sendo executado no IcePatch usando a variável de ambiente `APATCH`. Se estiver sendo executado no IcePatch, esse valor será definido como `true`.
:::
...
Você também pode declarar uma variável chamada `REMOVE` contendo uma lista de diretórios em `customize.sh` para executar operações de remoção, e o IcePatch executará automaticamente `mknod <TARGET> c 0 0` nos diretórios correspondentes do módulo. Por exemplo:
...
Você pode declarar uma variável chamada `REPLACE` em seu arquivo `customize.sh`, que inclui uma lista de diretórios a serem substituídos, e o IcePatch executará automaticamente as operações correspondentes em seu diretório de módulo. Por exemplo:
...
Um instalador do módulo IcePatch é um módulo IcePatch empacotado em um arquivo ZIP que pode ser atualizado no gerenciador do IcePatch. O formato deste arquivo ZIP é o seguinte:
...
::: warning AVISO
O módulo IcePatch **NÃO** é compatível para instalação no Recovery personalizado!
:::
...
O script `customize.sh` é executado no shell BusyBox `ash` do IcePatch com o Modo Autônomo ativado. As seguintes variáveis ​​e funções estão disponíveis:

#### Variáveis {#variables}

- `KERNELPATCH` (bool): Marque este script para rodar no ambiente do IcePatch, e o valor desta variável será sempre `true`.
...
- `APATCH` (bool): Marque este script para rodar no ambiente do IcePatch, e o valor desta variável será sempre `true`.
- `APATCH_VER_CODE` (int): Número da versão atual do IcePatch (ex.: `10672`).
- `APATCH_VER` (string): O nome da versão atual do IcePatch (ex.: `10672`).

- `BOOTMODE` (bool): No IcePatch, esta variável sempre terá o valor `true`.
...
::: warning AVISO
No IcePatch, `MAGISK_VER_CODE` é sempre `27000` e `MAGISK_VER` é sempre `v27.0`.
:::
...
No IcePatch, os scripts são divididos em dois tipos com base em seu modo de execução: modo post-fs-data e modo de serviço late_start:
...
No IcePatch, os scripts de inicialização são divididos em dois tipos com base no local de armazenamento: scripts gerais e scripts de módulo.
...
Todos os scripts de inicialização serão executados no shell BusyBox `ash` do IcePatch com o Modo Autônomo ativado.
