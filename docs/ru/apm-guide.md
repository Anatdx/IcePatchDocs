...
IcePatch предоставляет модульный механизм (AndroidPatch Module) для модификации системного раздела с сохранением его целостности. Этот механизм часто называют бессистемным (`systemless`).

Реализация модулей IcePatch скопирована и модифицирована из [KernelSU](https://github.com/tiann/KernelSU).
...
3. Поддержка SELinux, IcePatch напрямую использует `magiskpolicy`

Механизм работы модулей IcePatch почти такой же, как и Magisk. Если вы знакомы с разработкой модулей Magisk, то разработка модулей IcePatch очень похожа. Представление модулей ниже можно пропустить, достаточно прочитать в чем заключаются различия.

## BusyBox

IcePatch предоставляет полнофункциональный бинарный файл BusyBox (включая полную поддержку SELinux). Исполняемый файл находится по адресу `/data/adb/ap/bin/busybox`.
BusyBox от IcePatch поддерживает переключаемый во время выполнения «ASH Standalone Shell Mode».
...
Каждый сценарий оболочки, запущенный в контексте IcePatch, будет выполняться в оболочке BusyBox `ash` с включенным автономным режимом. Для сторонних разработчиков это касается всех загрузочных скриптов и скриптов установки модулей.

Для тех, кто хочет использовать эту функцию «автономного режима» вне IcePatch, есть два способа включить ее:
...
Чтобы гарантировать, что все последующие оболочки `sh` будут выполняться в автономном режиме, первый способ является предпочтительным (это также метод, используемый IcePatch и менеджером IcePatch), поскольку переменные окружения наследуются в дочерних процессах.
...
BusyBox от IcePatch теперь представляет собой бинарник, скомпилированный непосредственно с помощью проекта Magisk, **спасибо Magisk!**
Поэтому вам не нужно беспокоиться о совместимости скриптов BusyBox со скриптами Magisk и IcePatch, потому что они абсолютно одинаковы!
:::

## Модули APM {#IcePatch-modules}

Модуль IcePatch - это папка внутри `/data/adb/modules` со следующей структурой:
...
::: tip Различия с Magisk
IcePatch не имеет встроенной поддержки Zygisk, поэтому в модуле нет содержимого, связанного с Zygisk.
...
Вы можете определить, запущен ли скрипт в IcePatch, используя переменную окружения `APATCH`, если он запущен в IcePatch, это значение будет установлено в `true`.
:::
...
Вы также можете выполнить операцию удаления, объявив переменную `REMOVE` в `customize.sh` со списком директорий, и IcePatch автоматически выполнит за вас `mknod <TARGET> c 0 0` в соответствующей директории модуля. Например:
...
Вы можете объявить в файле `customize.sh` переменную с именем `REPLACE`, содержащую список заменяемых каталогов, и IcePatch автоматически выполнит соответствующие операции в каталоге вашего модуля. Например:
...
Пакет установки модуля IcePatch представляет собой zip-файл, который можно прошить через IcePatch Manager, и формат этого zip-файла следующий:
...
:::warning
Модуль IcePatch не поддерживается для установки в Recovery!
:::
...
Сценарий `customize.sh` запускается в «автономном режиме» в оболочке BusyBox `ash` от IcePatch. Вы можете использовать следующие переменные и функции:

#### Переменные {#variables}

- `KERNELPATCH` (bool): Пометьте этот скрипт для запуска в среде IcePatch, и значение этой переменной всегда будет `true`.
...
- `APATCH` (bool): Пометьте этот скрипт для запуска в среде IcePatch, и значение этой переменной всегда будет `true`.
- `APATCH_VER_CODE` (int): Номер текущей версии IcePatch (например. `10672`).
- `APATCH_VER` (string): Имя текущей версии IcePatch (например. `10672`).

- `BOOTMODE` (bool): В IcePatch эта переменная всегда будет иметь значение `true`.
...
::: warning
В IcePatch `MAGISK_VER_CODE` имеет значение `27000`, а `MAGISK_VER` - `27.0`.
:::
...
В IcePatch существует два типа скриптов в зависимости от режима их работы: режим post-fs-data и режим late_start service.
...
В IcePatch есть еще два типа стартовых скриптов в зависимости от места их хранения: общие скрипты и скрипты модулей.
...
Все сценарии запуска будут выполняться в оболочке BusyBox `ash` от IcePatch с включенным «автономным режимом».
