...
3. [Нажмите сюда](https://github.com/Anatdx/IcePatch/releases), чтобы получить последнюю стабильную версию IcePatch Manager.
...
2. IcePatch всегда патчит `boot.img` любого устройства. Не пытайтесь патчить или прошивать `init_boot` или другие файлы образов разделов. Разработчики IcePatch не несут ответственности за сбой патча и загрузки устройства в таком случае.
3. Избегайте использования файла `boot.img`, который был пропатчен другими людьми, чтобы избежать непредвиденных ситуаций.
:::

## Требования к установке

Требования к установке IcePatch в основном отражены в конфигурации ядра. Вот требования к ядру:
...
:::warning
Строго запрещено устанавливать слабые ключи, такие как `12345678`. Последние версии IcePatch [требуют использования сильных ключей](/ru/warn).
:::
...
Когда KernelPatch обновляется, а IcePatch Manager остается неизменным, вы можете вручную пропатчить ядро.
...
::: info
Вы можете нажать [сюда](https://exame.icepatch.anatdx.com/), чтобы попробовать.
:::
...
ПОСЛЕДНЯЯ версия IcePatch поддерживает прямую прошивку через стороннее Recovery (например, TWRP).

::: warning
Прямая прошивка впервые представлена в версии `10888`, и более ранние версии IcePatch **НЕ** поддерживают этот метод.
:::

Измените расширение файла IcePatch Manager (.apk) на `.zip`. Например:
...
После этого вы можете прошить этот файл `.zip` через функцию Flash стороннего Recovery. IcePatch будет автоматически установлен, как и Magisk.
...
Если вам нужно настроить SuperKey, пожалуйста, зайдите в IcePatch manager после загрузки и перепатчите, чтобы сбросить SuperKey.
:::
...
::: warning
Автоматическое удаление впервые представлено в версии `10888`, и более ранние версии IcePatch **НЕ** поддерживают этот метод.
:::

Измените расширение файла IcePatch manager (.apk) на `.zip` и добавьте `uninstall` в имя файла. Например:
...
После этого вы можете прошить этот файл `.zip` через функцию Flash стороннего Recovery. IcePatch будет автоматически удален, как и Magisk.
