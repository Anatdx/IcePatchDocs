...
IcePatch основан на KernelPatch, он наследует все его функциональные возможности и был расширен. Следовательно, IcePatch также поддерживает модули KernelPatch (KPM). Вот введение об использовании KPM.

## Что такое KPM?

Модули KernelPatch (KPM) - это тип модулей, который позволяет коду выполняться в пространстве ядра, подобно загружаемым модулям ядра (LKM). Они могут выполнять некоторые операции, которые APM не могут делать (например, защита образов разделов). Вы даже можете изменить ksud KernelSU на KPM, чтобы разрешить KernelSU работать на IcePatch.
...
В настоящее время IcePatch не реализовал функцию "Установка" для KPM, и вы можете использовать KPM только с помощью "Встраивания" и "Загрузки". Разработчики KernelPatch и IcePatch работают над быстрой реализацией функции "Установка". Пожалуйста, наберитесь терпения.
:::
...
Встраивание KPM может быть выполнено как при первом патчинге `boot.img`, так и после установки IcePatch.
...
#### Встраивание KPM после установки IcePatch

После установки IcePatch способ встраивания KPM аналогичен установке APM. Вы можете нажать кнопку в правом нижнем углу и выбрать "Embed". Оставшиеся шаги можно найти в разделе [Встраивание KPM при первом патчинге](/ru/kpm-usage-guide#embed-kpms-at-first-patching).
...
::: tip Внимание
KernelPatch и IcePatch еще не реализовали функцию "Установка" для KPM. Все описания ниже являются предположительными и описывают ожидаемое поведение "Установки" для KPM.
:::
